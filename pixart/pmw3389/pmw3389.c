...
static void set_interrupt(const struct device *dev, const bool en) {
    LOG_INF("In pwm3389_set_interrupt");
    const struct pixart_config *config = dev->config;
...
static enum pixart_input_mode get_input_mode_for_current_layer(const struct device *dev) {
    LOG_INF("In get_input_mode_for_current_layer");
//    const struct pixart_config *config = dev->config;
...
static int set_cpi_if_needed(const struct device *dev, uint32_t cpi) {
    LOG_INF("In pwm3389_set_cpi_if_needed");
    struct pixart_data *data = dev->data;
...
static int pmw3389_report_data(const struct device *dev) {
    LOG_INF("In pwm3389_report_data");
    struct pixart_data *data = dev->data;
...
//        TOINT16((buf[PMW3610_Y_L_POS] + ((buf[PMW3610_XY_H_POS] & 0x0F) << 8)), 12) / dividor;
    ...
    int16_t raw_x = ((int16_t)sys_get_le16(buf[PMW3389_DX_POS])) / CONFIG_PMW3389_CPI_DIVIDOR;
    int16_t raw_y = ((int16_t)sys_get_le16(buf[PMW3389_DY_POS])) / CONFIG_PMW3389_CPI_DIVIDOR;
    int16_t x;
    int16_t y;
...
static void pmw3389_gpio_callback(const struct device *gpiob, struct gpio_callback *cb,
                                  uint32_t pins) {
    LOG_INF("In pwm3389_gpio_callback");
    struct pixart_data *data = CONTAINER_OF(cb, struct pixart_data, irq_gpio_cb);
    const struct device *dev = data->dev;
...
static void pmw3389_work_callback(struct k_work *work) {
    LOG_INF("In pwm3389_work_callback");
